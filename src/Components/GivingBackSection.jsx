...
const GivingBackSection = () => {
    ...
    const Strip = ({ items }) => (
        <div className="w-full max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-10 mt-5">
            {items.map((item, index) => (
                <div
                    key={item.id}
                    className="relative flex flex-col items-center justify-center bg-gray-100 rounded-lg shadow-md p-6 text-center
                               transition-all duration-300 transform group hover:scale-105 hover:z-10
                               hover:bg-[#1f2a44] hover:text-white"
                >
                    {/* Divider line (only desktop) */}
                    {index !== 0 && (
                        <div className="hidden lg:block absolute left-0 top-6 bottom-6 w-px bg-gray-300 pointer-events-none"></div>
                    )}
